...
4. **[PRDCT-12071](https://app.clickup.com/t/86bbj56qh)** — *Product defect: posture issues re-affirm Open
   ...
   absence-adjacent close hook.
   **Ticket currency (`amendment_r50` part 4), posted this round.** The ticket's *Mechanism* section describes
   the frozen base: it says every re-evaluation that still MATCHES *"rewrites the evidence columns and bumps
   `updated_at`"*. That is no longer true at head — upstream `main` (`ac7f9de777`, `d7ae29286a`, `366ab0c488`,
   all merged into this branch) made the instance-write path **diff-guarded and counted**
   (`issue_repository.py:1316-1329`). A correcting comment saying so — and saying, equally plainly, that the
   **defect itself is unchanged** (Open forever, deleted content as live Evidence, no close-on-absence, no
   staleness surfacing) — was posted on the ticket and then **fetched back live** to prove it exists:
   [comment 90140246067955](https://app.clickup.com/t/86bbj56qh?comment=90140246067955), captured with
   per-element substring checks in [`evidence/at7-52-posture-ticket-correction.txt`](https://raw.githubusercontent.com/onyxsecurity/pywebagent/prdct11935-evidence/evidence/prdct11935/at7-52-posture-ticket-correction.txt)
   (15 checks, 0 failures). The re-run of at10's C3-audit check (q) against the live task is
   [`evidence/at10-28-check-q-rerun.txt`](https://raw.githubusercontent.com/onyxsecurity/pywebagent/prdct11935-evidence/evidence/prdct11935/at10-28-check-q-rerun.txt). Measured on head in [`evidence/at7-09-db-rows-posture.txt`](https://raw.githubusercontent.com/onyxsecurity/pywebagent/prdct11935-evidence/evidence/prdct11935/at7-09-db-rows-posture.txt), shown pre-existing
